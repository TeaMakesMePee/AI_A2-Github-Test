...
void SceneKnight::Render()
{
	...
	RenderMesh(meshList[GEO_CHESSBOARD2], false);
	...
	for (int i = 0; i < m_grid.size(); ++i)
	{
		...
		{
			modelStack.PushMatrix();
			modelStack.Translate(((i - 5 * (i / 5)) + 0.5f) * m_gridSize, (i / 5 + 0.5f) * m_gridSize, 5.f);
			modelStack.Scale(m_gridSize - 1.f, m_gridSize - 1.f, m_gridSize);
